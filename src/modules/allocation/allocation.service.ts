...
export const allocationService = {
	async preview(
		...
		for (const direction of ["outbound", "return"] satisfies FlightDirection[]) {
			...
			plan.unassigned.push(...directionPlan.unassigned.map((row) => ({ ...row, direction })))
		...
		return db.transaction(async (tx) => {
			...
			for (const row of assignments) {
				...
				if (next > target.capacity) {
					throw new ConflictError(`Chuyến ${target.code} không còn đủ chỗ. Hãy chạy preview lại.`)
				}
...
async function previewVehicles(eventId: string, actor: AuditActor): Promise<AllocationRunRow> {
	...
	for (let index = 0; index < TRANSPORT_LEGS.length; index++) {
		...
		const legPlan = allocateVehicles({
			...
			lockedAssignments: currentAssignments
				...
				.map((row) => ({
					registrationId: row.assignment.registrationId,
					vehicleId: row.assignment.vehicleId,
				})),
		})
		plan.assignments.push(
			...legPlan.assignments.map((row) => ({
				registrationId: row.registrationId,
	...
	return db.transaction(async (tx) => {
		const run = await allocationRepository.insert(
			{ id, eventId, type: "vehicle", params: {}, stats, plan, createdBy: actor.id },
			tx,
		)
		await auditService.record(
			{
...
async function commitVehicles(
	...
	const lockedKeys = new Set(
		current
			.filter((row) => row.assignment.locked)
			.map((row) => `${row.assignment.registrationId}:${row.assignment.leg}`),
	)
	const occupancy = new Map<string, number>()
	...
	for (const leg of TRANSPORT_LEGS) {
		candidateSets.set(
			leg,
			new Set((await vehicleRepository.listCandidates(eventId, leg, tx)).map((row) => row.id)),
		)
	}
	const assignments = plan.assignments.filter(
		(row) => !lockedKeys.has(`${row.registrationId}:${row.leg}`),
	)
	for (const row of assignments) {
		...
		if (!target || target.leg !== row.leg)
			throw new ConflictError("Dữ liệu xe đã thay đổi. Hãy preview lại.")
		const next = (occupancy.get(target.id) ?? 0) + 1
		if (next > target.capacity)
			throw new ConflictError(`Xe ${target.code} không còn đủ chỗ. Hãy preview lại.`)
		occupancy.set(target.id, next)
	}
