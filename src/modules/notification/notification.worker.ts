...
async function markDeliveryFailure(notif: NotificationRow, error: string): Promise<void> {
...
async function processNotification(notif: NotificationRow): Promise<void> {
	...
	try {
		switch (notif.template) {
			...
			case "assignment_changed": {
				const changeType = (notif.payload as { changeType?: string }).changeType as
					"flight" | "vehicle" | "room" | "schedule" | "announcement"
		...
		logger.error("notification.render_failed", {
			id: notif.id,
