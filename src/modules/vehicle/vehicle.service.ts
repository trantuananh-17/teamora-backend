...
import type {
	...
	ListVehiclesQuery,
	ManualAssignVehicleInput,
	SetVehicleAssignmentLockInput,
	UpdateVehicleInput,
} from "./vehicle.dto"
...
export const vehicleService = {
	...
	async create(eventId: string, input: CreateVehicleInput, actor: AuditActor) {
		...
		if (await vehicleRepository.findByCodeLeg(eventId, code, input.leg))
			throw new ConflictError(`Đã có xe ${code} cho chặng này.`)
	...
	async update(eventId: string, id: string, input: UpdateVehicleInput, actor: AuditActor) {
		...
		if (next.departAt < next.gatherAt)
			throw new ValidationError("Giờ khởi hành không được trước giờ tập trung.")
		if (next.capacity < before.assignedCount)
			throw new ConflictError(
				`Xe đang có ${before.assignedCount} người, không thể giảm còn ${next.capacity} chỗ.`,
			)
		...
		if (collision && collision.id !== id)
			throw new ConflictError(`Đã có xe ${next.code} cho chặng này.`)
		const affected = (await vehicleRepository.listAssignments(eventId))
			.filter((row) => row.assignment.vehicleId === id)
		...
		return db.transaction(async (tx) => {
			...
			await auditService.record(
				{ eventId, actor, entity: ENTITY, entityId: id, action: "update", before, after },
				tx,
			)
	...
	async delete(eventId: string, id: string, actor: AuditActor) {
		...
		if (before.assignedCount > 0)
			throw new ConflictError("Không thể xóa xe đang có người được phân bổ.")
		await db.transaction(async (tx) => {
			if (!(await vehicleRepository.delete(eventId, id, tx))) throw new NotFoundError("Vehicle")
			await auditService.record(
				{ eventId, actor, entity: ENTITY, entityId: id, action: "delete", before },
				tx,
			)
		})
	},
	async listAssignments(
		eventId: string,
		query: ListVehicleAssignmentsQuery,
		pagination: PaginationQuery,
	) {
		const legs = query.leg
			? [query.leg]
			: ([
					"origin_to_airport",
					"airport_to_hotel",
		...
		const [assignments, ...candidateLists] = await Promise.all([
			...
			...legs.map((leg) => vehicleRepository.listCandidates(eventId, leg)),
		])
		const byKey = new Map(
			assignments.map((row) => [`${row.assignment.registrationId}:${row.assignment.leg}`, row]),
		)
		let items = candidateLists.flatMap((rows, index) =>
			rows.map((candidate) => {
	...
	async manualAssign(eventId: string, input: ManualAssignVehicleInput, actor: AuditActor) {
		return db.transaction(async (tx) => {
			...
			const candidates = await vehicleRepository.findCandidatesByIds(
				eventId,
				target.leg,
				input.registrationIds,
				tx,
			)
			if (candidates.length !== new Set(input.registrationIds).size)
				throw new ValidationError("Có CBNV không cần xe ở chặng này hoặc không thuộc kỳ.")
			const before = await vehicleRepository.findAssignmentsForRegistrations(
				eventId,
				target.leg,
				input.registrationIds,
				tx,
			...
			if (finalOccupancy > target.capacity)
				throw new ConflictError(
					`Xe ${target.code} không đủ chỗ; cần thêm ${finalOccupancy - target.capacity} chỗ.`,
				)
			await vehicleRepository.upsertManualAssignments(
				eventId,
				target.id,
				target.leg,
				actor.id,
			...
			const after = await vehicleRepository.findAssignmentsForRegistrations(
				eventId,
				target.leg,
				input.registrationIds,
				tx,
	...
	async setAssignmentLock(
		eventId: string,
		id: string,
		input: SetVehicleAssignmentLockInput,
		actor: AuditActor,
	) {
		return db.transaction(async (tx) => {
	...
	async exportWorkbook(eventId: string, actor: AuditActor) {
		...
		await auditService.record({
			...
			entityId: eventId,
			action: "export",
			after: { vehicles: vehicles.length, assignments: assignments.length },
		})
		return writeWorkbook([
			{
				name: "Xe",
				columns: [
					{ key: "code", header: "Mã xe", required: true },
					{ key: "name", header: "Tên xe", required: true },
					{ key: "leg", header: "Chặng", required: true },
					{ key: "capacity", header: "Sức chứa", required: true },
					{ key: "assigned", header: "Đã xếp", required: true },
					{ key: "gatherAt", header: "Giờ tập trung", required: true },
					{ key: "departAt", header: "Giờ khởi hành", required: true },
					{ key: "destination", header: "Điểm đến", required: true },
					{ key: "leaderName", header: "Trưởng xe", required: false },
					{ key: "leaderPhone", header: "Điện thoại", required: false },
				],
				rows: vehicles.map((row) => ({
					...
					gatherAt: formatDate(row.gatherAt),
					departAt: formatDate(row.departAt),
			...
			},
			{
				name: "Phân xe",
				columns: [
					{ key: "name", header: "CBNV", required: true },
					{ key: "email", header: "Email", required: true },
					{ key: "team", header: "Team", required: true },
					{ key: "leg", header: "Chặng", required: true },
					{ key: "vehicle", header: "Xe", required: true },
					{ key: "source", header: "Nguồn", required: true },
					{ key: "locked", header: "Đã khóa", required: true },
				],
				rows: assignments.map((row) => {
					const person = candidates.get(row.assignment.registrationId)
					return {
						name: person?.user.name ?? row.assignment.registrationId,
						email: person?.user.email ?? "",
						team: person?.team.name ?? "",
						leg: row.assignment.leg,
						vehicle: row.vehicle.code,
						source: row.assignment.source,
						locked: row.assignment.locked ? "Có" : "Không",
					}
				}),
			},
		])
