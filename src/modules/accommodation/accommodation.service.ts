...
export const accommodationService = {
	...
	listRoomTypes: (eventId: string, hotelId?: string) =>
		accommodationRepository.listRoomTypes(eventId, hotelId),
	listRooms: (eventId: string, query: ListRoomsQuery) =>
		accommodationRepository.listRooms(eventId, query),
	...
	async createHotel(eventId: string, input: CreateHotelInput, actor: AuditActor) {
		if (await accommodationRepository.findHotelByName(eventId, input.name))
			throw new ConflictError("Kỳ này đã có khách sạn cùng tên.")
		return db.transaction(async (tx) => {
			const created = await accommodationRepository.insertHotel(eventId, input, tx)
			await auditService.record(
				{ eventId, actor, entity: "hotel", entityId: created.id, action: "create", after: created },
				tx,
			)
			return created
		})
	...
	async updateHotel(eventId: string, id: string, input: UpdateHotelInput, actor: AuditActor) {
		...
		return db.transaction(async (tx) => {
			...
			await auditService.record(
				{ eventId, actor, entity: "hotel", entityId: id, action: "update", before, after },
				tx,
			)
			return after
		})
	...
	async deleteHotel(eventId: string, id: string, actor: AuditActor) {
		const before = await accommodationRepository.findHotel(eventId, id)
		if (!before) throw new NotFoundError("Hotel")
		if ((await accommodationRepository.listRooms(eventId, { hotelId: id })).length)
			throw new ConflictError("Hãy xóa các phòng trước khi xóa khách sạn.")
		await db.transaction(async (tx) => {
			await accommodationRepository.deleteHotel(eventId, id, tx)
			await auditService.record(
				{ eventId, actor, entity: "hotel", entityId: id, action: "delete", before },
				tx,
			)
		})
	},
	async createRoomType(
		...
		if (!(await accommodationRepository.findHotel(eventId, hotelId)))
	...
	async updateRoomType(eventId: string, id: string, input: UpdateRoomTypeInput, actor: AuditActor) {
		...
		return db.transaction(async (tx) => {
			...
			await auditService.record(
				{ eventId, actor, entity: "room_type", entityId: id, action: "update", before, after },
				tx,
			)
			return after
		})
	...
	async deleteRoomType(eventId: string, id: string, actor: AuditActor) {
		...
		if (!before) throw new NotFoundError("Room type")
		if (
			(await accommodationRepository.listRooms(eventId, { hotelId: before.hotelId })).some(
				(row) => row.room.roomTypeId === id,
			)
		)
			throw new ConflictError("Loại phòng đang được sử dụng.")
		await db.transaction(async (tx) => {
			await accommodationRepository.deleteRoomType(eventId, id, tx)
			await auditService.record(
				{ eventId, actor, entity: "room_type", entityId: id, action: "delete", before },
				tx,
			)
		})
	},
	async createRoom(eventId: string, hotelId: string, input: CreateRoomInput, actor: AuditActor) {
		...
		if (!type || type.hotelId !== hotelId)
			throw new ValidationError("Loại phòng không thuộc khách sạn này.")
		return db.transaction(async (tx) => {
			const created = await accommodationRepository.insertRoom(eventId, hotelId, input, tx)
			await auditService.record(
				{ eventId, actor, entity: "room", entityId: created.id, action: "create", after: created },
				tx,
			)
			return created
		})
	...
	async updateRoom(eventId: string, id: string, input: UpdateRoomInput, actor: AuditActor) {
		...
		if (input.capacity !== undefined && input.capacity < before.assignedCount)
			throw new ConflictError(`Phòng đang có ${before.assignedCount} người.`)
		if (input.roomTypeId) {
			const type = await accommodationRepository.findRoomType(eventId, input.roomTypeId)
			if (!type || type.hotelId !== before.room.hotelId)
				throw new ValidationError("Loại phòng không thuộc khách sạn này.")
		}
		return db.transaction(async (tx) => {
			...
			await auditService.record(
				{ eventId, actor, entity: "room", entityId: id, action: "update", before, after },
				tx,
			)
			return after
		})
	...
	async deleteRoom(eventId: string, id: string, actor: AuditActor) {
		...
		await db.transaction(async (tx) => {
			...
			await auditService.record(
				{ eventId, actor, entity: "room", entityId: id, action: "delete", before },
				tx,
			)
		})
	},
	async importAssignments(eventId: string, fileName: string, buffer: Buffer, actor: AuditActor) {
		...
		const [participants, rooms, current] = await Promise.all([
			accommodationRepository.listParticipants(eventId),
		...
		const outcome = validateRoomAssignments(
			sheet.rows.map((row) => ({
				...
				hotelName: row.values.hotelName ?? "",
				roomCode: row.values.roomCode ?? "",
			})),
			participants,
			rooms.map((row) => ({
				roomId: row.room.id,
			...
			current
				.filter((row) => row.assignment.source === "manual")
				.map((row) => ({ registrationId: row.assignment.registrationId, roomId: row.room.id })),
		)
		if (!outcome.ok)
			throw new ValidationError(`File có ${outcome.errors.length} lỗi. Chưa ghi dòng nào.`, {
				totalErrors: outcome.errors.length,
				errors: outcome.errors,
			})
		await db.transaction(async (tx) => {
			await accommodationRepository.replaceImportedAssignments(
				eventId,
				actor.id,
				outcome.records,
				tx,
			...
			await auditService.record(
				{
					...
					entityId: eventId,
					action: "import",
					after: { fileName, total: outcome.records.length },
				},
				tx,
			)
			await notificationService.scheduleChangesIfPublished(
				eventId,
				outcome.records.map((row) => row.registrationId),
				"room",
				tx,
			)
		})
	...
	async exportWorkbook(eventId: string, actor: AuditActor) {
		...
		await auditService.record({
			...
			entityId: eventId,
			action: "export",
			after: { hotels: hotels.length, rooms: rooms.length, assignments: assignments.length },
		})
		const hotelById = new Map(hotels.map((row) => [row.hotel.id, row.hotel]))
		return writeWorkbook([
			// Importable sheet first: the reader only looks at worksheets[0].
			{
				name: "Phân phòng",
				columns: roomAssignmentColumns,
			...
			},
			{
				name: "Khách sạn",
				columns: [
					{ key: "name", header: "Khách sạn", required: true },
					{ key: "address", header: "Địa chỉ", required: true },
					{ key: "roomCount", header: "Số phòng", required: true },
				],
				rows: hotels.map((row) => ({
					name: row.hotel.name,
			...
			},
			{
				name: "Loại phòng",
				columns: [
					{ key: "hotel", header: "Khách sạn", required: true },
					{ key: "name", header: "Loại phòng", required: true },
					{ key: "capacity", header: "Sức chứa mặc định", required: true },
				],
				rows: roomTypes.map((row) => ({
					hotel: hotelById.get(row.hotelId)?.name ?? "",
					name: row.name,
			...
			},
			{
				...
				columns: [
					{ key: "hotel", header: "Khách sạn", required: true },
					{ key: "code", header: "Mã phòng", required: true },
					{ key: "type", header: "Loại phòng", required: true },
					{ key: "capacity", header: "Sức chứa", required: true },
					{ key: "assigned", header: "Đã phân", required: true },
				],
				rows: rooms.map((row) => ({
					hotel: row.hotel.name,
					code: row.room.code,
					type: row.roomType.name,
					capacity: row.room.capacity,
