...
export const accommodationRepository = {
	async listHotels(eventId: string, executor: DbExecutor = db) {
		return executor
			.select({ hotel, roomCount: count(room.id) })
			.from(hotel)
			.leftJoin(room, eq(hotel.id, room.hotelId))
			.where(eq(hotel.eventId, eventId))
			.groupBy(hotel.id)
	...
	async findHotel(eventId: string, id: string, executor: DbExecutor = db) {
		return (
			await executor
				.select()
				.from(hotel)
				.where(and(eq(hotel.eventId, eventId), eq(hotel.id, id)))
				.limit(1)
		)[0]
	},
	async findHotelByName(eventId: string, name: string, executor: DbExecutor = db) {
		return (
			await executor
				.select()
				.from(hotel)
				.where(and(eq(hotel.eventId, eventId), eq(hotel.name, name)))
				.limit(1)
		)[0]
	},
	...
	async deleteHotel(eventId: string, id: string, executor: DbExecutor = db) {
		return (
			(await executor.delete(hotel).where(and(eq(hotel.eventId, eventId), eq(hotel.id, id))))
				.rowsAffected > 0
		)
	},
	async listRoomTypes(eventId: string, hotelId?: string, executor: DbExecutor = db) {
		return executor
			...
			.from(roomType)
			.where(
				and(eq(roomType.eventId, eventId), hotelId ? eq(roomType.hotelId, hotelId) : undefined),
			)
	...
	async findRoomType(eventId: string, id: string, executor: DbExecutor = db) {
		return (
			await executor
				.select()
				.from(roomType)
				.where(and(eq(roomType.eventId, eventId), eq(roomType.id, id)))
				.limit(1)
		)[0]
	},
	async insertRoomType(
		eventId: string,
		hotelId: string,
		...
		return (
			await executor
				.insert(roomType)
				.values({ id: newId(), eventId, hotelId, ...input })
				.returning()
		)[0]!
	},
	...
	async listRooms(eventId: string, query: ListRoomsQuery = {}, executor: DbExecutor = db) {
		...
		if (query.search)
			conditions.push(
				or(ilike(room.code, `%${query.search}%`), ilike(hotel.name, `%${query.search}%`))!,
			)
		return executor
			.select({ room, hotel, roomType, assignedCount: count(roomAssignment.id) })
			.from(room)
			.innerJoin(hotel, eq(room.hotelId, hotel.id))
			.innerJoin(roomType, eq(room.roomTypeId, roomType.id))
			.leftJoin(roomAssignment, eq(room.id, roomAssignment.roomId))
	...
	async findRoom(eventId: string, id: string, executor: DbExecutor = db) {
		return (
			await executor
				.select({ room, assignedCount: count(roomAssignment.id) })
				.from(room)
				.leftJoin(roomAssignment, eq(room.id, roomAssignment.roomId))
				.where(and(eq(room.eventId, eventId), eq(room.id, id)))
				.groupBy(room.id)
				.limit(1)
		)[0]
	},
	async insertRoom(
		...
		return (
			await executor
				.insert(room)
				.values({ id: newId(), eventId, hotelId, ...input, code: input.code.toUpperCase() })
				.returning()
		)[0]!
	},
	...
	async deleteRoom(eventId: string, id: string, executor: DbExecutor = db) {
		return (
			(await executor.delete(room).where(and(eq(room.eventId, eventId), eq(room.id, id))))
				.rowsAffected > 0
		)
	},
	...
	async listAssignments(eventId: string, executor: DbExecutor = db) {
		return executor
			...
			.from(roomAssignment)
			.innerJoin(room, eq(roomAssignment.roomId, room.id))
			.innerJoin(hotel, eq(room.hotelId, hotel.id))
			.innerJoin(registration, eq(roomAssignment.registrationId, registration.id))
			.innerJoin(user, eq(registration.userId, user.id))
			.leftJoin(employeeProfile, eq(user.id, employeeProfile.userId))
			.where(eq(roomAssignment.eventId, eventId))
			.orderBy(asc(hotel.name), asc(room.code), asc(user.name))
	},
	async replaceImportedAssignments(
		eventId: string,
		actorId: string,
		rows: { roomId: string; registrationId: string }[],
		executor: DbExecutor = db,
	) {
		await executor
			.delete(roomAssignment)
			.where(and(eq(roomAssignment.eventId, eventId), eq(roomAssignment.source, "import")))
