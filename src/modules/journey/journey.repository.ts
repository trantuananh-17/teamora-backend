...
export const journeyRepository = {
		...
		return (
			await executor
				.select({
					...
					profile: { employeeCode: employeeProfile.employeeCode, phone: employeeProfile.phone },
		...
		return executor
			...
			.where(
				and(
					...
					eq(flightAssignment.registrationId, registrationId),
				),
			)
			.orderBy(asc(flight.departAt))
	},
	vehicles(eventId: string, registrationId: string, executor: DbExecutor = db) {
		return executor
			.select({
				...
				vehicle,
				pickupPoint: { id: pickupPoint.id, name: pickupPoint.name, address: pickupPoint.address },
			})
			.from(vehicleAssignment)
			.innerJoin(vehicle, eq(vehicleAssignment.vehicleId, vehicle.id))
			.leftJoin(pickupPoint, eq(vehicle.pickupPointId, pickupPoint.id))
			.where(
				and(
					eq(vehicleAssignment.eventId, eventId),
					eq(vehicleAssignment.registrationId, registrationId),
				),
			)
			.orderBy(asc(vehicle.departAt))
	},
	async accommodation(eventId: string, registrationId: string, executor: DbExecutor = db) {
		return (
			(
				await executor
					.select({ assignment: roomAssignment, room, roomType, hotel })
					.from(roomAssignment)
					...
					.where(
						and(
							eq(roomAssignment.eventId, eventId),
							eq(roomAssignment.registrationId, registrationId),
						),
					)
					.limit(1)
			)[0] ?? null
	...
	announcements(eventId: string, participating: boolean, executor: DbExecutor = db) {
		...
		return executor
			...
			.where(
				and(
					eq(announcement.eventId, eventId),
					inArray(announcement.audience, [...audiences]),
	...
	async dashboard(eventId: string, executor: DbExecutor = db) {
		const [employeeTotal, registrations, transport, flights, vehicles, rooms] = await Promise.all([
			executor
				...
				.where(eq(employeeProfile.active, true)),
			executor
				.select({
					participating: registration.participating,
					shift: registration.shiftPreference,
					value: count(),
				})
				.from(registration)
				.where(eq(registration.eventId, eventId))
				.groupBy(registration.participating, registration.shiftPreference),
			executor
				.select({
					leg: registrationTransportNeed.leg,
					needed: registrationTransportNeed.needed,
					value: count(),
				})
				.from(registrationTransportNeed)
				.innerJoin(registration, eq(registrationTransportNeed.registrationId, registration.id))
				.where(eq(registration.eventId, eventId))
				.groupBy(registrationTransportNeed.leg, registrationTransportNeed.needed),
			executor
				.select({
					id: flight.id,
					code: flight.code,
					...
					assigned: count(flightAssignment.id),
				})
				.from(flight)
			...
			executor
				.select({
					...
					assigned: count(roomAssignment.id),
				})
				.from(room)
				.innerJoin(hotel, eq(room.hotelId, hotel.id))
				.leftJoin(roomAssignment, eq(room.id, roomAssignment.roomId))
				.where(eq(room.eventId, eventId))
				.groupBy(room.id)
				.orderBy(asc(hotel.name), asc(room.code)),
		])
		return {
			employeeTotal: employeeTotal[0]?.value ?? 0,
			registrations,
			transport,
			flights,
			vehicles,
