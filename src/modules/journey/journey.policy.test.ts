...
describe("journey publication policy", () => {
	it.each(["registration_open", "registration_closed", "allocation_processing"] as const)("hides allocations at %s", (status) => {
		expect(canViewJourney(status)).toBe(false)
	})
	it.each(["information_published", "event_started", "event_completed"] as const)("shows allocations at %s", (status) => {
